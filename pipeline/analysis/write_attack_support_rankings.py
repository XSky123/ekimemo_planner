...
def has_default_level(item: dict[str, Any], level: str = "50") -> bool:
    try:
        levels = json.loads(str(item.get("level_data") or "{}"))
    except json.JSONDecodeError:
        return False
    return level in levels


def visible_counts(candidates: list[dict[str, Any]], level: str = "50") -> tuple[int, int, int]:
    visible = sum(1 for item in candidates if not item.get("vu_only") and has_default_level(item, level))
    vu_only = sum(1 for item in candidates if item.get("vu_only"))
    other_hidden = len(candidates) - visible - vu_only
    return visible, vu_only, other_hidden


def tab_count_html(candidates: list[dict[str, Any]]) -> str:
    visible, vu_only, other_hidden = visible_counts(candidates)
    parts = [f'<span class="count-main">{visible}</span>']
    if vu_only:
        parts.append(f'<span class="vu-count">+{vu_only} VU</span>')
    return "".join(parts)


def section_count_html(candidates: list[dict[str, Any]]) -> str:
    visible, vu_only, other_hidden = visible_counts(candidates)
    suffixes = []
    if vu_only:
        suffixes.append(f"+{vu_only} VU")
    suffix = " " + " ".join(suffixes) if suffixes else ""
    return f'<span class="muted">({visible}{suffix})</span>'
...
def render_table(tab_id: str, candidates: list[dict[str, Any]]) -> str:
    ...
    <section class="tab-panel" id="panel-{esc(tab_id)}" data-tab-panel="{esc(tab_id)}">
      <h2>{esc(tab["title"])} {section_count_html(candidates)}</h2>
...
"""
    ...
    write_text_lf(OUT_HTML, html_text)
