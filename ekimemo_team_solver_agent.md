...
3. 解析相关でんこ名称与别名，查 profile cache。
4. 如果没有，进入 fill 模式抓取相关个别详情页。
5. 建立目标 profile / scenario profile / mechanism profile。
6. 从 reverse_index cache、role_profiles、indexes 找相关候选组件。
7. 如果 reverse_index 或必要 index 不存在，抓取并缓存。
8. reverse_index 只作为候选发现，不直接确认最终条件。
9. 对候选读取 `skill_parsed` 与 `role_profiles` cache；缺失时先生成通用 `candidate_role_profile`。
10. 用通用画像做粗评分和场景过滤，保留 Top 10～20 + 条件启用/冲突候选。
11. 对保留候选计算或读取目标适配分；进入 Pareto 候选、缺少 `detail_reviewed: true`、或置信度低时必须复核个别详情页。
12. 应用 manual_overrides，并记录依据。
